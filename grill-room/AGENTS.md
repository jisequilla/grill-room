...
| `find-superseded` | Ask the interviewer which open loose ends a decision that settled later has already answered, and which settled decisions a decision that settled later replaced, and store what it finds on each as a proposal. A pending proposal never blocks confirmation. Runs automatically as the second half of every done proposal that has either to check; this action repeats it on demand. Allowed while interviewing or with a done proposal pending, refused while a turn is working. |
...
<root>/<workingExportFolder>/<folderName>/issues/NN-slug.md   # "Blocked by: NN, NN" line
...
- Bundle paths are stored as `{{BUNDLE}}` and filled in at export from a
  fresh `git check-ignore` of the bundle folder (the stored visibility flag
  when git gives no answer; the flag itself only drives the settings page
  and the mismatch warning): `tracked` gives paths relative to the repo root
  plus a commit-before-delegating step (and a push, on the pull-request
  recipe); `ignored` gives absolute paths into the main checkout and tells
  worktree agents to read the bundle by absolute path. A repository with no
  commits is described as greenfield: make the first commit before
  delegating.
