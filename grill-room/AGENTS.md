...
| `find-superseded` | Ask the interviewer which open loose ends a decision that settled later has already answered, which settled decisions a decision that settled later replaced, and which settled own answers postpone their question rather than decide it (a deferral), and store what it finds on each as a proposal. A pending proposal never blocks confirmation. Runs automatically as the second half of every done proposal that has any of the three to check; this action repeats it on demand. Allowed while interviewing or with a done proposal pending, refused while a turn is working. |
