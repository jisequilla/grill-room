...
export default defineAction({
  description:
    "Preview a session's export without writing anything: the proposed slug (first four words of the title), the slug used, the folder name the project's slug pattern resolves to, the absolute bundle directory, every file that will be written (absolute paths: decisions.md when the tree holds decisions or out-of-scope items, HANDOFF.md and briefs/NN-slug.md when a handoff has been generated, and the export manifest), the files the previous manifest lists that will be removed, the project's tracker diagnostic, and the export gate: `exportBlocked` and `exportBlockedReason` (`handoff-missing` or `handoff-stale`, null once a current handoff exists) that export-session actually refuses on, so the UI can explain it before the operator tries. Built by the same plan export-session writes, so the two cannot disagree.",
  schema: z.object({
