...
export default defineAction({
  description:
    "Re-check a session's export visibility without exporting again: classifies every file the same export would write (or already wrote) as tracked, ignored, or untracked in the target repository, with a remedy when agents will not see it and a warning when the project's visibility flag disagrees with what was observed. Takes the same sessionId and slug as preview-export and export-session, and builds the same plan, so it always asks about the files the current export would touch — whether or not export-session has run yet.",
  schema: z.object({
