...
export default defineAction({
  description:
    "What registering a folder would detect, without registering it: the git root it resolves to, a default name, a verify command suggested from the repository's justfile, package.json or Makefile, a working export folder and slug pattern suggested from a declared tracker block when the repository has a valid one, and — given a working export folder — the visibility git check-ignore seeds for it. Refuses a folder outside any git repository.",
  schema: z.object({
    folder: z
      .string()
      .min(1)
  ...
  http: { method: "GET" },
  run: async ({ folder, workingExportFolder }) => {
    const outcome = await inspectProjectFolder(folder, workingExportFolder);
