 ...
 * beside the metrics it reads, editing `metrics.ts` to add the event it
 * emits, and ticket 2, blocked by 1, wires that alert into the metrics
 * module and proves it with a test of its own. Ticket 2's dependency on
 * ticket 1 is the edited-file form (`editedPath` + `symbol`: the event ticket
 * 1 adds to `metrics.ts`), so the result exercises that `buildsOn` shape —
 * the created-path form is exercised by `test-fixtures.ts`'s
 * `aHandoffScoutResult`, used elsewhere. Cites the same fixture paths as
        ...
        tickets: [
          ...
          },
          {
            ...
            buildsOn: [
              {
                blocker: 1,
                provides: "The lag event ticket 1 adds to metrics.ts.",
                citation: null,
                createdPath: null,
                editedPath: "src/ingest/metrics.ts",
                symbol: "emitLagEvent",
                check: "grep -n emitLagEvent src/ingest/metrics.ts",
              },
            ],
            provedBy: {
