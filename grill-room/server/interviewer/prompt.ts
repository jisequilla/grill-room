...
function buildHandoffScoutPrompt(request: HandoffScoutRequest): string {
  ...
  return [
    ...
    "  scoped to provide, the statement says so and names the consequence for",
    "  this ticket: a response it must return that the code cannot yet",
    "  express, or a field the spec assumes that does not exist. A statement",
    "  about a whole file or component cites the full range it describes, or",
    "  it is split into facts that each carry their own citation.",
    ...
    "  project already runs its tests. The test file is one of this ticket's",
    "  own `filesToChange`, as a `create` or an `edit`: the ticket may only",
    "  write the files it lists. Only a ticket that changes no files may name",
    "  a test outside them. `testPath` is a test in the project's own test",
    "  layout, never a source file the ticket changes. `command` must run",
    "  that test and fail without this ticket's change; narrow it to that",
    "  test file or package where the project's runner allows one. A",
    "  project-wide command that runs every test — the same command every",
    "  brief already runs — is not a proof on its own. When no existing test",
    "  covers the ticket's acceptance criteria, add one: list it as a",
    "  `create` in `filesToChange` and name it here as `testPath`. A ticket",
    "  proved only by a build — codegen, configuration — gives the build",
    "  command, and as `testPath` the test among its files that exercises",
