...
function buildHandoffScoutPrompt(request: HandoffScoutRequest): string {
  ...
  return [
    ...
    "  tests for this ticket's kind of change (see below). The `command`",
    "  runs it, in the form the project already runs its tests.",
    ...
    "  acceptance criteria, add one: list it as a `create` in",
    "  `filesToChange` and name it here as `testPath`. A ticket proved only",
    "  by a build — codegen, configuration — gives `command` as the build",
    "  command followed by the command that runs the test named in",
    "  `testPath`, for example `<build> && <test command>`, so `command`",
    "  exercises the test it names whenever `testPath` is set.",
    ...
    "  default discovery. The scout confirms this and states it in a",
    "  fact, citing the config or recipe line that collects it. With no",
    "  explicit include globs, it cites the line that invokes the runner",
    '  (e.g. `package.json`\'s `"test": "vitest run"`) and names the',
    "  runner's default pattern in the fact's text. This fact may cite a",
    "  config or recipe file the ticket does not change, an exception to",
    "  facts being about the code the ticket touches. A path the runner",
    "  does not collect — a file under `scripts/` when the runner only",
    "  globs `src/**/*.test.ts` — is not a proof: pick a path the runner",
    "  collects.",
    "",
    ...CITATION_RULES,
